...
completions until the end: the user watches the items settle individually. In
Claude Code, prefer the **background watcher** (next section) to re-arm the
loop — it survives turn boundaries, which `ffb_wait` does not. For clients that
do not support long-polling, call `ffb_pull` once when the user says they sent
feedback, then use the same per-item completion loop. Codex and Cursor need a
one-time `mcp add` registration for this server; they do not need to add it for
each use.
